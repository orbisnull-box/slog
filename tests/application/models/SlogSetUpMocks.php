...
class SlogSetUpMocks extends OrbisLib_SetUpMocks
{
    ...
    public function mockCommentMapper()
    {
        $mock = $this->setMock("Application_Model_CommentMapper", "Application_Model_DbTable_Comment");
        $testCase = $this->getTestCase();
        $adapter = $this->getAdapter();
        ...
        unset($testDataNew["id"]);
        $testDataNew["created"]=date("Y-m-d h:i:s");
        $testDataNew["entry"]=$testDataNew["entry"]->id;
